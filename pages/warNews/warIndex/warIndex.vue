...
<script>
	...
	export default {
		...
		mounted() {
			this.getIndexInfo();
			eventBus.$on('reachBottomWarIndex', (size) => {
				this.getIndexInfo(size);
			})
		},
		destroyed() {
			eventBus.$off('reachBottomWarIndex')
		},
		methods: {
			async getIndexInfo(size = 0) {
				const res = await this.$API.warApi.getIndexInfo(size);
				const data = JSON.parse(res.slice(res.indexOf('(') + 1, res.lastIndexOf(')')));
				this.cardList = [...this.cardList, ...data.BAI67OGGwangning]
